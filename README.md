...
floodnet-energy-cadence/
  ...
  figures/                   the five poster figures (plus fig1's derived CSV side
                             output), regenerated by scripts/
  docs/                      EVIDENCE_41_PERCENT.md (proof chain for the headline claim)
                             and OTII_RECORDING_NAMES.md (per-recording documentation)
  images/                    measurement-setup screenshot and photo, poster preview
```

Contents are Python files (.py), Markdown (.md), JSON (.json), CSV (.csv), PNG/JPEG images,
one PowerPoint file (.pptx), and one PDF. Total size without raw data: about 10 MB.
...
Each figure regenerates with one command from the repository root:
...
(`open_meteo_2025-07-01_2026-07-01.json`, with a manifest recording the endpoint, retrieval
time, and SHA-256), the derived daily CSV, and the full provenance record
...
Verified identity: the linked PDF's SHA-256 is
`5EC577B952B9DC5106593A4E4C3A6E8CEA64F6F72B8FAAF1BB512155C29DC516`, byte-identical to the
copy the analysis used, so a checksum after download confirms you are reading the same
document.
